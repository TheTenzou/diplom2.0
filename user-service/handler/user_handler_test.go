...
func TestDeleteUser(test *testing.T) {
	...
	test.Run("Success", func(test *testing.T) {
		...
		mockUserService := new(mocks.MockUserService)
		mockUserService.On("Delete", mock.AnythingOfType("*context.emptyCtx"), userID).Return(mockUserResponse, nil)
	...
	test.Run("NotFound", func(test *testing.T) {
		...
		mockErr := apperrors.NewNotFound("user", userID.Hex())
		mockUserService := new(mocks.MockUserService)
		mockUserService.On("Delete", mock.Anything, userID).Return(model.User{}, mockErr)
