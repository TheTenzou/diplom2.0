...
class ScheduledTaskServiceImp(
    ...
    override fun createScheduledTask(scheduledTaskDto: ScheduledTaskRequestDto): ScheduledTaskDetailDto {
        val scheduleOptional = scheduleRepository.findById(scheduledTaskDto.schedule)
        if (scheduleOptional.isEmpty) {
            throw EntityNotFoundException("Schedule with id: ${scheduledTaskDto.schedule} doesn't exist")
        }
        val schedule = scheduleOptional.get()

        val tsoddOptional = tsoddRepository.findById(scheduledTaskDto.tsodd)
        if (tsoddOptional.isEmpty) {
            throw EntityNotFoundException("Tsodd with id: ${scheduledTaskDto.tsodd} doesn't exist")
        }
        val tsodd = tsoddOptional.get()

        val taskTypeOptional = taskTypeRepository.findById(scheduledTaskDto.taskType)
        if (taskTypeOptional.isEmpty) {
            throw EntityNotFoundException("Task type with id: ${scheduledTaskDto.taskType} doesn't exist")
        }
        val taskType = taskTypeOptional.get()

        val crewOptional = crewRepository.findById(scheduledTaskDto.crew)
        if (crewOptional.isEmpty) {
            throw EntityNotFoundException("Crew with id: ${scheduledTaskDto.crew} doesn't exist")
        }
